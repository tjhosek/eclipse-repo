...
public class Lab4Exercises {
	...
	public static int romanToInt(String roman)
	{
		...
		//Ensuring the given roman numeral is in uppercase
		...
		{
			//Changing char variables
			previousChar = currentChar;
			currentChar = roman_upper.charAt(i);
			
			// Adds a different value to the number dependant on the value of currentChar and previousChar. if previousChar would decrease the value of
			// the roman numeral, double the amount is subtracted. This is because the loop would have added the value of the character when it went over it
			// the first time
			switch(currentChar)
			{
	...
	{
		/**
		 * Starts a Menu which will print different patterns dependent on user input
		 */
		...
		while(isRunning) 
		{
			...
			//Gets the option and stores it in a char value
			char option = menu.next().trim().toUpperCase().charAt(0);
			int size = 0;
			switch(option) 
			{
			case 'A' : 
			...
			case 'C' : 
				...
				printDiamond(size);
				break;
			case 'E' : isRunning = false; break; // Exit the menu
			default: System.out.println("Please input a valid option."); break; // Invalid choice
			}
		}
		menu.close();
	}
	...
	{
		/**
		 * Prints an hourglass shape made of descending numbers of a given size
		 */
	...
	{
		/**
		 * Prints a triangle of asterisks of a given size
		 */
	...
	{
		/**
		 * Prints one row of the diamond printed in printDiamond()
		 */
		//Printing empty space
		for(int j = size - i; j > 0; j--)
		{
	...
	public static void randomFrequency()
	{
		...
		for(int i = 0; i < 10; i++)
		{
			...
			//Adding to the correct frequency
			switch(number)
			{
			case 1 : ones++; break;
			case 2 : twos++; break;
			...
			//Adding the numbers to the string
		...
		//Checking which number has the highest frequency. By doing it this way if there is a tie the lower number will be chosen
		...
		//Removing the number with the highest frequency
		if(highestFrequency == ones)
		{
			numbers = numbers.replaceAll("1,", "");
			numbers = numbers.replaceAll("1", "");
			numbers = numbers.replaceAll("0", "10"); //Makes sure that 10s stay as 10s and aren't accidentally tuned into 0s
		}
	...
	public static void drawAudiLogo()
	{
		...
		EllipseComponent logo = new EllipseComponent();
		frame.add(logo);
